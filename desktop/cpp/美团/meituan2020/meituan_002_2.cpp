...
int main() {
    ...
    for (int i = n - 1; i >= 0; --i) {
        ...
        if (d.end() != left && d.end() != right) {  // 将俩个区间合并为一个
            ...
            // 哈希表删除与新增
            d.erase(lr); d.erase(rl);
            d[ll] = d[rr] = {ll, rr, sum};
            // multiset删除与新增
            s.erase(lsum); s.erase(rsum); s.insert(sum);
        } else if (d.end() != left) {  // 和左侧区间合并
            int ll = left->second.left, lr = left->second.right, lsum = left->second.sum;
